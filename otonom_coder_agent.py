#!/usr/bin/env python
"""
Qwen3-Coder + Ollama ile otonom coder agent (plan → apply + test fix, v4).
...
* (Optionally) test command and its failing stdout/stderr.

You must:

* Understand the task and the planner's intent (if given).
* If test failure info is provided, focus on fixing the failure while preserving existing working code as much as possible.
* Decide the minimal set of files to create/update/delete to complete the task.
* Output a JSON array describing the file operations.
...
def build_coder_messages(
    ...
    test_failure_info: Optional[str] = None,
) -> List[Dict[str, Any]]:
    file_tree = get_workspace_overview(workspace, cfg.general.max_files_in_tree)
...
# ---------------------------------------------------------------------------
# Planner + Coder + Test ana fonksiyon
# ---------------------------------------------------------------------------
...
def run_agent_round(
    ...
    - Aksiyon listesini ve birleşik özet metnini döndürür
    """
    workspace.mkdir(parents=True, exist_ok=True)
    endpoint = select_endpoint(config, mode_override)

    # Dry-run & test parametreleri
    ...
    try:
        ...
        if planner_summary_text:
            summary_chunks.append("📦 Dosya Aksiyonları\n" + err_summary)
        else:
            summary_chunks.append(err_summary)
        final_summary = "\n\n".join(summary_chunks)
    ...
    if not dry_run_flag and run_tests_effective:
        ...
        failure_info_for_llm = test_summary
        ...
        while remaining_fixes > 0:
            ...
            fix_messages = build_coder_messages(
                ...
                cfg=config,
                planner_json_text=planner_json_text_for_coder,
                test_failure_info=failure_info_for_llm,
            )
            ...
            if rc2 == 0:
                # Başarılıysa döngü biter
                break

            # Hâlâ hata varsa ve tekrar deneyeceksek, yeni failure_info_for_llm güncellenir
