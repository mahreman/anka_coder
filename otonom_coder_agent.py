#!/usr/bin/env python
"""
Qwen3-Coder + Ollama ile otonom coder agent (plan → apply, v3).

Bu modül:
- Config'i okur
- PLAN turu (isteğe bağlı, max_rounds >= 2 ise)
- CODER turu (dosya aksiyonları üretir)
- İstenirse workspace içinde dosyaları GERÇEKTEN yazar/siler (dry-run destekli)
- Plan + aksiyon özetini string olarak döndürür (web UI'de chat mesajı olarak gösterilecek)
"""
...
def summarize_plan(plan_obj: Optional[Dict[str, Any]], raw_json_text: str) -> str:
    if plan_obj is None:
        # JSON parse edilemediyse kısa fallback
    ...
    if not lines:
        # JSON ayrıştı ama içi boş geldiyse, ham text'i kısaltıp göster
...
def summarize_actions(
    ...
    if not actions:
        ...
        if dry_run:
            return "🟡 DRY-RUN (dosyalara dokunulmadı)\n\n" + base
        return base
...
def run_agent_round(
    ...
    - Aksiyon listesini ve plan+aksiyon özetini döndürür
    """
    ...
    if not dry_run_flag:
        try:
            ...
            summary_parts_err: List[str] = []
            if planner_summary_text:
                summary_parts_err.append("📋 Plan\n" + planner_summary_text + "\n")
            summary_parts_err.append(
                "❌ Dosya işlemlerinde hata oluştu.\n\n"
                f"Hata: {e}"
            )
            final_summary_err = "\n".join(summary_parts_err)
            return actions, final_summary_err

    actions_summary = summarize_actions(actions, logs, dry_run=dry_run_flag)

    # Plan + aksiyon özetini birleştir
